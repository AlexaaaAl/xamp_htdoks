...
$config['smtp_from'] = 'Александра'; //От кого
  $number = $_POST["number"];
...
function smtpmail($to='', $mail_to, $subject, $message, $headers='') {
	...
	if (!server_parse($socket, "250", __LINE__)) {
		if ($config['smtp_debug']) echo '<p>Не могу отправить HELO!</p>'; /*выпадает!!!!!!!!!!!!!!!!*/
		fclose($socket);
		return false;
	}
...
/*отправка письма*/
smtpmail('Roket-Busines', 'rbru-metrika@yandex.ru', 'SMTP Отправка', $number);
